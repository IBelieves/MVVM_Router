...
public class DbController {
    ...
    public DbController(Context context) {
        ...
        recordEntityDao = mDaoSession.getRecordEntityDao();
    ...
    //--------记录事件-----
    //查询某个
    public RecordEntity recordSearch(long id) {
        List<RecordEntity> list = recordEntityDao.queryBuilder().where(RecordEntityDao.Properties.Id.eq(id)).list();

        return list == null || list.size() == 0 ? new RecordEntity() : list.get(0);
    }

    //获取全部
    public ArrayList<RecordEntity> recordAll() {
        List<RecordEntity> list1 = recordEntityDao.queryBuilder().list();


        return list1 == null ? new ArrayList() : (ArrayList<RecordEntity>) list1;
    }
    //获取最新20条
    public ArrayList<RecordEntity> recordLastNew() {
        List<RecordEntity> list = recordEntityDao.queryBuilder().orderDesc(RecordEntityDao.Properties.Create_time).offset(0).limit(20).list();

        return list == null ? new ArrayList() : (ArrayList<RecordEntity>) list;
    }

    //更新单个
    public void recordUpdates(RecordEntity data) {

        recordEntityDao.insertOrReplace(data);
    }

    //删除单个
    public void recordDelete(RecordEntity data) {
        recordEntityDao.delete(data);
    ...
    private class CustomOpenHelper extends DaoMaster.DevOpenHelper {
        ...
        public void onUpgrade(Database db, int oldVersion, int newVersion) {
            ...
            MigrationHelper.migrate(db, new MigrationHelper.ReCreateAllTableListener() {
                ...
                //每增加一个数据库，这里记得加上
            }, UserInfoEntityDao.class, RecordEntityDao.class);
        }
