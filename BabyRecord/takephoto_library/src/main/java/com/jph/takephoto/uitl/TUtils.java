...
public class TUtils {
    ...
    public static ArrayList<Uri> convertImageToUri(Context context,ArrayList<Image>images) throws TException {
        ...
        for(Image image:images){
            //@TODO("安卓手机29版本及以上路径处理)
            if (Build.VERSION.SDK_INT >= 29)
            image.path=    TUriParse.getFileWithUri(getImageContentUri_(context,image.path), (Activity) context).getAbsolutePath();
            uris.add(FileProvider.getUriForFile(context,TConstant.getFileProviderName(context), new File(image.path)));
        }
        return uris;
    }
    public static Uri getImageContentUri_(Context context, String path) {
     ...
     * @return
     */
    public static ArrayList<TImage> getTImagesWithImages(ArrayList<Image> images, TImage.FromType fromType, Activity activity){
        ArrayList<TImage>tImages=new ArrayList();
        for(Image image:images){
            //@TODO("安卓手机29版本及以上路径处理)
            if (Build.VERSION.SDK_INT >= 29)
            image.path=    TUriParse.getFileWithUri(getImageContentUri_(activity,image.path), activity).getAbsolutePath();
